...
if [[ ${enabled} == true ]]; then
    # shellcheck disable=SC1083,SC1054,SC1073,SC1065,SC1064,SC1072,SC1009
    {% set gke_data = componentMetadata.cloudProviderDetails.account.services | selectattr('category', 'eq', 'KUBERNETES') | list | first %}
    # shellcheck disable=SC1083,SC1054,SC1073,SC1065,SC1064,SC1072,SC1009
    {% set env_variables = gke_data.data.environmentVariables | default({}, true) %}
    # shellcheck disable=SC1083,SC1054,SC1073,SC1065,SC1064,SC1072,SC1009
    env_variables="{{ env_variables | tojson | replace('"', '\\"') }}"
    env_variables=$(echo "${env_variables}" | jq '. + {"APP_DIR": "/tmp/'"${artifact_name}"'", "DEPLOYMENT_TYPE": "gcp_container"}')
    if [[ -f "state.json" && $(jq -r '.deployConfig' state.json) != null ]]; then
    ...
    if [ "$1" == "post-deploy" ]; then
        ODIN_APPLICATION_DISCOVERY=$(bash discovery.sh)
        env_variables=$(echo "${env_variables}" | jq '. + {"ODIN_APPLICATION_DISCOVERY": '"${ODIN_APPLICATION_DISCOVERY}"'}')
    fi
