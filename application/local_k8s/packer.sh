...
if [ -f "manifest.json" ]; then
    ...
    DOCKER_FLAG=""
    if [[ $(echo "${DOCKER_REGISTRY_FOR_PUBLISHING}" | jq -r '.[0].data.insecure // false') == "true" ]]; then
      DOCKER_FLAG="--insecure"
    fi
    docker manifest create "${DOCKER_FLAG}" "${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG}" --amend "${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG}-amd64" --amend "${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG}-arm64"
    docker manifest push "${DOCKER_FLAG}" "${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG}"
