...
if [[ ${enabled} == true ]]; then
    ...
    {% set k8s_data = componentMetadata.cloudProviderDetails.account.services | selectattr('category', 'eq', 'KUBERNETES') | list | first %}
    ...
    env_variables=$(echo "${env_variables}" | jq '. + {"ODIN_APP_DIR": "/tmp/'"${ARTIFACT_NAME}"'", "ODIN_DEPLOYMENT_TYPE": "local_k8s"}')
    if [[ -f "state.json" && $(jq -r '.deployConfig' state.json) != null ]]; then
