...
if [[ "${LOCAL_ARTIFACT_ENABLED}" == "true" ]]; then
  ...
  if [[ -d ${ARTIFACT_CACHE_PATH} ]]; then
    ...
    export ARTIFACTORY_DATA=$(echo ${COMPONENT_METADATA} | jq '(.cloudProviderDetails.account | select(.services[]?.category == "STORAGE")) // (.cloudProviderDetails.linked_accounts[] | select(.services[]?.category == "STORAGE") // error("Service with category STORAGE not found in account or linked accounts."))')
