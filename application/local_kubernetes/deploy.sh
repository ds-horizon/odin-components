...
export DOCKER_REGISTRIES=$(echo "${COMPONENT_METADATA}" | jq '[.cloudProviderDetails.account.services[], (.cloudProviderDetails.linked_accounts[].services[])] | map(select(.category=="DOCKER_REGISTRY"))')
# shellcheck disable=SC2155
