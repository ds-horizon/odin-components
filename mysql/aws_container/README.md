...
| `imageTag`        | string             | **Yes**  | MySQL Docker image version tag. Controls the MySQL version deployed (e.g., 8.0.35, latest). Changing this triggers rolling updates and may require data migrations. Pin to specific versions for production stability. **Required:** Must be compatible with existing data. **Production:** Use specific version tags (avoid 'latest'), test upgrades in non-prod first.                                              |
