...
| `replicationGroupId`          | string                                 | **Yes**  | AWS resource identifier for the ElastiCache replication group. Becomes the permanent cluster identifier visible in AWS console and CLI. Cannot be changed after creation. Must be 1-40 alphanumeric characters or hyphens, start with a letter, no spaces or uppercase. Choose descriptive names following your naming convention (e.g., 'prod-cache-users'). **Required by AWS API**.                                                                                                                                  |
...
| `ipDiscovery`                 | string                                 | No       | IP protocol version for node discovery endpoints. IPv6 requires Redis 6.2-7.1 (Redis OSS) or 7.2+ (Valkey) on Nitro instances. Affects how clients resolve node addresses. Choose based on VPC configuration and client support. **Default: `'ipv4'`** (universal compatibility). **Production:** Use IPv6 only with full stack IPv6 support; test client library compatibility first. Possible values are: `ipv4`, `ipv6`.                                                                                             |
...
| `parameterGroupName`          | string                                 | No       | ElastiCache parameter group name containing Redis configuration settings. Controls advanced Redis parameters not exposed as direct properties. AWS provides defaults: `default.redis7` for Redis 7.x, `default.redis6.x` for 6.x, append `.cluster.on` for cluster mode. Create custom groups for fine-tuning. **Default:** AWS selects based on Redis version and cluster mode. **Production:** Create custom parameter groups for workload-specific tuning (timeout, maxmemory-policy, etc.).                         |
...
| `userGroupIds`                | string[]                               | No       | User group IDs for Redis ACL-based access control (RBAC). Requires Redis 6.0+ and authentication enabled. Each group contains users with specific command/key permissions defined via ACL rules. Enables fine-grained security beyond simple AUTH. **Default: `[]`** (no RBAC, uses AUTH only). **Production:** Implement least-privilege access with groups: 'read-only' for analytics, 'write-only' for producers, 'admin' for operations; regularly audit permissions.                                               |
